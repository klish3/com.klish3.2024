"use client";

export const Summary = () => {
  return (
    <>
      <div className="mb-6">
        <blockquote className="text-xl font-semibold italic text-gray-900 dark:text-white">
          <svg
            className="mb-4 h-8 w-8 text-gray-400 dark:text-gray-600"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 18 14"
          >
            <path d="M6 0H2a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h4v1a3 3 0 0 1-3 3H2a1 1 0 0 0 0 2h1a5.006 5.006 0 0 0 5-5V2a2 2 0 0 0-2-2Zm10 0h-4a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h4v1a3 3 0 0 1-3 3h-1a1 1 0 0 0 0 2h1a5.006 5.006 0 0 0 5-5V2a2 2 0 0 0-2-2Z" />
          </svg>
          <p className="mx-auto px-8">
            "I am an innovative and results-driven Technical Lead with over 10
            years of experience in front-end development and leadership."
          </p>

          <svg
            className="ml-auto h-8 w-8 text-gray-400 dark:text-gray-600"
            aria-hidden="true"
            xmlns="http://www.w3.org/2000/svg"
            fill="currentColor"
            viewBox="0 0 18 14"
          >
            <path d="M6 0H2a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h4v1a3 3 0 0 1-3 3H2a1 1 0 0 0 0 2h1a5.006 5.006 0 0 0 5-5V2a2 2 0 0 0-2-2Zm10 0h-4a2 2 0 0 0-2 2v4a2 2 0 0 0 2 2h4v1a3 3 0 0 1-3 3h-1a1 1 0 0 0 0 2h1a5.006 5.006 0 0 0 5-5V2a2 2 0 0 0-2-2Z" />
          </svg>
        </blockquote>
      </div>
      <p className="my-6 px-10 text-gray-900 dark:text-white">
        Proven expertise in TypeScript, React, React Native, Angular, and AWS
        services. Demonstrated success in implementing cutting-edge
        technologies, fostering team growth, and delivering high-impact
        solutions in fast-paced environments.
      </p>
    </>
  );
};
